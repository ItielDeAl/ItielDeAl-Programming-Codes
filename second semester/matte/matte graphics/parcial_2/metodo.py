...
# Definir la función y su derivada
    x = sp.symbols('x')
    y = sp.sympify(entrada_funcion.get())
    derivada = sp.diff(y, x)


    def newton_raphson(x0, tolerancia = entrada_error.get(), max_iter = 30, error_absoluto = float('inf')):
...
# Definir el valor inicial y ejecutar el método
    x0 = float(entrada_aprox.get()) # Convertir a float
# Llamar a la función
    newton_raphson(x0)
...
resultado3 = tk.Label(frame_resultados, text="", font=("Arial", 12))
resultado3.grid(row=1, column=2, padx=5)

resultado4 = tk.Label(frame_resultados, text="", font=("Arial", 12))
resultado4.grid(row=1, column=3, padx=5)
...
boton = tk.Button(ventana, text="Mostrar resultados", command=metodo)
boton.pack(pady=10)
